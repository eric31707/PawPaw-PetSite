...
CalendarPicker.prototype._insertCalendarIntoWrapper = function () {
    ...
    var handleSelectedElement = (event) => {
        if (event.target.nodeName.toLowerCase() === this.calendarDayElementType && !event.target.classList.contains('disabled')) {
            ...
            var calendar = $("#calendar").prev().children().text()
            
            var calendarlist = calendar.split("-")
            var year = calendarlist[1].trim()
            var month = calendarlist[0].substring(0, 1)
            ...
            if (event.target.classList.contains("selected"))
            {
                ...
                if (startdate == "") {
                    ...
                    $(event.target).prevAll("time").addClass("disabled");
                    event.target.classList.add('selected');
                    $("#startdate").text(year + "-" + month + "-" + startdate)                  
                }
                else
                {
                    enddate = event.target.textContent;
                    $(event.target).nextAll("time").addClass("disabled");
                    event.target.classList.add('selected');
                    $("#enddate").text(year + "-" + month + "-" + enddate)
                }
            ...
            if (startdate != "" && enddate != "")
            {
                for (var i = 0; i < parseInt(enddate) - parseInt(startdate); i++)
                {
                    $(event.target).prevAll("time").addClass('selected');
                }
            }
